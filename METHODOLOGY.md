...
- **Panel completeness is enforced, not assumed.** A cross-lab panel is only meaningful if the labs are actually present, and the ensemble is deliberately tolerant: a vendor whose key is absent does not join, and a judge that errors mid-run is dropped so a single outage cannot sink a grade. Left alone, that tolerance is a hole — a three-lab panel grades happily and its output is shaped exactly like a four-lab one. A grading run can therefore declare the panel it requires (`PROVING_GROUND_REQUIRE_JUDGES`), and a missing lab aborts the run instead of quietly producing a thinner grade. Every judgment additionally records the configured panel alongside the judges that actually returned, so any published grade states how many labs produced it and which, if any, were missing. A grade produced by three labs is never mistakable for one produced by four, including long after the fact.
